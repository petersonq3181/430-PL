...
(define-type ExprC (U NumC IdC String IfC AppC LamC))
...
(define (parse [s : Sexp]) : ExprC
  (match s
    ...
    [(? symbol? s) #:when (boolean? (member s '(vars: body: if proc go))) (IdC s)]
    ...
    [(list 'proc (list arguments ...) 'go e) (cond
                                               [(validate-args arguments) (define args
                                                                            (cast arguments (Listof Symbol)))
                                             (cond
                                               [(symbol-repeats? args)
                                                (error 'parse "JYSS5: function cannot have duplicate par names")]
                                               [else (LamC args (parse e))])]
                                               [else (error 'parse "JYSS5: function pars are not symbols")])]
    
    [(list 'vars: v ... 'body: b ...) (define vv (cast v (Listof Sexp)))
                                      (define bb (cast b (Listof Sexp)))
                                      (define body (cast (first bb) Sexp))
                                      (AppC
                                       (LamC (map (lambda (i)
                                                    (define ii (cast i (Listof Sexp)))
                                                    (cast (first ii) Symbol)) vv) (parse body))
                                       (map (lambda (i)
                                              (define ii (cast i (Listof Sexp)))
                                              (parse (cast (first (rest (rest ii))) Sexp))) vv))]

    [(list b ...) (define a (cast b (Listof Sexp)))
                  (match (first a)
                    [(? symbol? s) (AppC (parse (first a))
                                (map (lambda (i) (parse (cast i Sexp))) (rest a)))]
                    [other (error 'parse "JYSS5: AppC must start with a symbol")])]))
...
(check-exn (regexp (regexp-quote "JYSS5: AppC must start with a symbol"))
           (lambda () (parse '{3 4 5})))
